...
const PopularBarcelona = () => {
  ...
  const getRestaurants = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8000/rating_restaurants/Barcelona/?page=${page}`);
  ...
  return (
    <div>
      ...
      <RestaurantContainer>
      {restaurants.slice(displayStartIndex, displayStartIndex + 4).map((restaurant, index) => {
        ...
        return (
          ...
          <Image src={photoUrl} alt={restaurant.name} onClick={() => {
            console.log(restaurant.id); // Imprime el id del restaurante
