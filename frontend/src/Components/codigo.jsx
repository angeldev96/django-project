...
const Header = () => {
  ...
  const handleLogin = async (userToken, ownerAccess, ownerRefresh) => {
    ...
    console.log('User token:', userToken); // Imprime el token de usuario
    localStorage.setItem('userToken', userToken);
    localStorage.setItem('ownerAccess', ownerAccess);
    console.log ('owner access token:', ownerAccess); // Imprime el token de acceso del propietario
    console.log('Owner refresh token:', ownerRefresh); // Imprime el token de actualización del propietario
    localStorage.setItem('ownerRefresh', ownerRefresh);
    setUserToken(userToken);
    setOwnerAccessToken(ownerAccess);
    setOwnerRefreshToken(ownerRefresh);
    // Obtén la información del usuario y del propietario aquí
    if (userToken) {
      const userInfo = await getUserInfo();
      if (userInfo && userInfo.is_owner) {
        // Si el usuario es un propietario, establece currentOwner y currentUser a null
        setCurrentOwner(userInfo);
        setCurrentUser(null);
        setIsOwner(true); // Set isOwner to true if user is an owner

      } else {
        // Si el usuario no es un propietario, establece currentUser y currentOwner a null
        setCurrentUser(userInfo);
        setCurrentOwner(null);
      }
    }
    if (ownerAccess) {
      const ownerInfo = await getOwnerInfo(ownerAccess); // Pasa ownerAccess directamente
      // Si el propietario inicia sesión, establece currentOwner y currentUser a null
      setCurrentOwner(ownerInfo);
      setCurrentUser(null);
      setIsOwner(true); // Set isOwner to true here
    }
    console.log('Current owner after login:', currentOwner); // Agrega esto
    console.log('isOwner:', isOwner); // Imprime el estado isOwner
    console.log('currentOwner:', currentOwner); // Imprime el estado currentOwner
  };
